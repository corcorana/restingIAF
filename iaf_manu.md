...
![*Fig_s6_paf_poz.* Power spectral density (PSD) plots displaying average variance of the EEG signal recorded over POz for frequency bins resolved in the range of 1 to 25 Hz. PSD estimates derived from a 2 min eyes-closed resting-state EEG recording. Light grey column signifies alpha band interval, which constitutes the peak frequency search window. *Left panel*: Linear scaled PSD. Strong alpha band activity is evidenced by a sharp component spanning ~7.5 to 12.5 Hz, and peaking at ~9.75 Hz. *Right panel*: Alternative depiction of left panel PSD in which ordinate data have been base-10 log-transformed (note, some researchers likewise tranform the frequency axis when depicting log-transformed PSDs). In this case, log-transformation has the effect of accentuating the relatively minor peak detected in the beta range of the spectrum (this activity approximates the first harmonic of the dominant alpha rhythm).](s6_paf_poz.png){#s6_paf_poz}
