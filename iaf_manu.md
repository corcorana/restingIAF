...
If different paradigms do result in distinct patterns of ERD [or indeed,  *increase* the synchronisation of alpha rhythm generators; see for e.g., @rihs2007] during the selected test interval, then the ensuing responsiveness regions used to define the coverage of the CoG estimate will span non-identical frequency bands [cf. @haegens2014, for evidence of analogous intraindividual shifts in PAF in response to varying task conditions]. 
