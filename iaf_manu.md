...
That is, in the case where the PSD function declines approximately monotonically (e.g., conforms to the $1/f$-like power distribution without showing the characteristic alpha-band deviation), the highest power value within $A$ will be the supremum encountered at the first frequency bin searched within the alpha band interval.
...
Synthetic resting-state EEG data were generated by combining a sine wave time series oscillating at some alpha band frequency with a time series whose frequency distribution broadly conformed to the $1/f$ inverse power-law scaling characteristic of background M/EEG activity [@novikov1997;@pritchard1992].
