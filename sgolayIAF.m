...
%
% AC, UniSA Magill, 31/01/2017. Programme developed in MATLAB 2014a
%% Required functions:
%   alphaParams
%   gravity
%   meanChansSG
...
n = 30;             % number of subjects to include
...
% minPower = 1;       % minimum power threshold that peak must surpass to be considered candidate alpha peak (not just noise)
...
%f_int = pow2(floor(log2(length(0:Fs/h:Fs/2)))) + 1;     % number of freq
%bins generated by pwelch routine (depricated)
cmin = 3;           % minimum number of channels with clear peaks required to compute cross-channel average estimate of alpha peak

% bounds of frequency range included in analysis - set to filter passband
% freqL = .3; - don't have f vector yet, and finding nearest f bin may
% result in unpredictable f bounds - initialised matrices may not match.
% just take 0 - 30, filtering will affect all spectra equally anyway
freqU = 30;
% numFrex = length(0:Fs/h:freqU);
...
psd = struct('pxx', [], 'd0', [], 'd1', [], 'd2', [], 'selP', [], 'selG', [], 'avPeak', [], 'sdPeak', [], 'avSpec', [], 'rmseSpec', [], 'avSmoo', [], 'CoG', []);      % this will collect PSD estimates & derivatives
...
for idx_i = 1:n     % for each i-th subject
    ...
    for idx_j = 1:2       % for each j-th resting-state recording
        ...
        for idx_k = 1:EEG.nbchan
            ...
            p = polyfit(f, psd(idx_i, idx_j, idx_k).pxx, 1);        % fit 1st order poly (regression line) to normalised spectra
            minPow = polyval(p, f);                                 % derive coefficients of fitted polynomial
        ...
        [ gravs(idx_i, :, idx_j), psd(idx_i, idx_j).selG ] = gravity([psd(idx_i, idx_j, :).d0], f, f1(idx_i, :, idx_j), f2(idx_i, :, idx_j));
                          
        % calculate average pt estimates/spectra across k-th channels for each j-th recording
        [ psd(idx_i, idx_j).selP, psd(idx_i, idx_j).qWt, psd(idx_i, idx_j).avPeak, psd(idx_i, idx_j).sdPeak, cog(idx_i, idx_j), psd(idx_i, idx_j).avSpec, psd(idx_i, idx_j).rmseSpec, psd(idx_i, idx_j).avSmoo ] = meanChansSG(gravs(idx_i, :, idx_j), psd(idx_i, idx_j).selG, peaks(idx_i, :, idx_j), Qf(idx_i, :, idx_j), psd, idx_i, idx_j, EEG.nbchan, cmin); 
                    
    end         % end j-th recording loop

    % average mean PSD estimates across (j-th) recordings
    [mspec, msmoo, mu_cog(idx_i, :)] = wtMean(idx_i, psd, EEG.nbchan, cog(idx_i,:));
    ...
    iaf(idx_i,7) = nanmean([ psd(idx_i, 1).avPeak, psd(idx_i, 2).avPeak ]);
