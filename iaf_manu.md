...
The definition of both alpha band and $\varphi$, the discrimination threshold, pose non-trivial problems (more on which shortly).
...
Moreover, it is well documented that a sizeable proportion of individuals fail to manifest an unambiguous PAF, on account of there being either two (or more) peaks within the alpha band [so-called 'split-peaks'; @chiang2011], or a general absence of prominent alpha-band activity [e.g., @anokhin1996] (see [Fig_bad_pafs](#bad_pafs)).
...
First, it fails to distinguish spectra featuring singular, dominant peaks from those possessing prominent secondary peaks (i.e. where visual inspection of the plot would suggest two or more alpha peaks that differ by some -- potentially trivial -- magnitude; e.g., left and central panels, [Fig_bad_pafs](#bad_pafs)).
...
The fabricated pink noise time series was multiplied with the target alpha signal to generate a composite signal, which was then subjected to PSD estimation and smoothing (see [Fig_sim_peaks](#sim_peaks) for an illustration of the signal generation procedure).
...
<!---IBS: What about age effects? I think that we should at least look into / report these (at least as supplementary materials), as age is an obvious source of inter-individual variability here 
AC: I'll figure out a way to fit this in when I get a bit of time over the weekend -->
...
Kernel density estimation of the probability density function underlying $PAF_{GM}$ data suggested that sample estimates were well-characterised by a Gaussian distribution ([Fig_GA_distribs](#GA_distribs)).
...
As a final point of comparison with established findings, we examined the relation between age and IAF.
...
![*Fig_age.* Scatterplots displaying association between grand averaged PAF/CoG estimates and age (years). Red curve: 2nd-degree polynomial fit.](figs/age.eps){#age}
...
The SNR = 0.30 example displayed in [Fig_snr](#snr) reveals one instance where the application of the smoothing procedure to a reasonably blunt component results in the erroneous ascription of PAF to a neighbouring frequency bin.
...
The second situation issues from the suboptimal recapitulation of the target component by `pwelch`, which may result in either a broad structure featuring two local maxima (e.g., SNR = 0.10, [Fig_snr](#snr)) or a more clearly defined split-peak (e.g., SNR = 0.20, [Fig_snr](#snr)).
...
By the same token, it is also reassuring that neither of the two cases in which the technique failed to extract PAF estimates demonstrated compelling evidence of any concerted alpha peak activity on visual inspection of the relevant PSD plots ([Fig_no_pafs](#no_pafs)).
...
Although a high proportion of channel data derived across both pre- and post-experiment recordings were available for grand averaging in most cases, individuals for whom only a subset of channel estimates could be derived tended to demonstrate stronger alpha peak activity in the latter EEG recording (see [Fig_stacked_chans](#stacked_chans)).
