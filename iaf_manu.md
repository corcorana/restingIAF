...
Synthetic resting-state EEG data were generated by combining a time series comprising at least one alpha frequency component with a time series whose frequency distribution broadly conformed to the $1/f$ power law characteristic of background EEG activity.
This latter background or pink noise series were produced using the `pinknoise` function (Zhivomirov, 2013).
This programme works by generating a vector of Gaussian-distributed random values, transforming the series into the frequency domain for $1/f$ scaling, and then converting the series back into the time domain following mean-centring and unity-based normalisation.
Alpha-band time series were generated by combining a vector of sine waves centred at $f_c$ Hz with a vector of Gaussian-distributed random numbers (s.d. = 0.5).
The pink noise and alpha component series were then convolved in preparation for PSD estimation and smoothing.
[Strictly speaking, time series were multiplied together in the time domain prior to PSD estimation. 
Note however that multiplication in the time domain is equivalent to convolution in the frequency domain; @lyons2004.]
All synthetic signals were constructed to replicate the 250 Hz sampling rate and 2 min duration of the empirical data reported above.

#### 2.4.2 Preliminary analysis
As an initial proof of concept, we investigated the capacity of the SGF technique to extract PAF estimates from spectra containing a single alpha-band component amidst varied levels of SNR.
In order to generate a randomly distributed set of alpha-band signals, we randomly sampled (with replacement) a vector of $f_c$ values that ranged from 7.5 to 12.5 Hz in iterations of 0.1 Hz.
Each sampled $f_c$ was used to synthesise an alpha component time series (as described in the preceding section) that was convolved with an independently generated pink noise signal.
This process was repeated 100 times for each level of SNR analysis.

SNR was controlled by varying the proportion of the alpha component signal included in the final time series.
For instance, if the desired SNR were .5, the full 2 min time series would be convolved with the pink noise signal.
However, if the desired SNR were .25, samples from the second half of the alpha series would set to 1.
This effectively ensured that the first half of the final time series was the convolute of the original two signals, whereas the remainder of the series  comprised solely of pink noise fluctuations.
...
<!-- Zhivomirov, H. (2013). pinknoise [Freeware]. Retrieved from https://au.mathworks.com/matlabcentral/fileexchange/42919-pink--red--blue-and-violet-noise-generation-with-matlab-implementation/content/pinknoise.m (Accessed 18/03/2017) -->