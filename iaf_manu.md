...
![*Fig_f1f2.* Fixed vs. individually adapted alpha frequncy band windows. *Left panel*: Individual bandwidth (indicated by shaded area under PSD) closely approximates the standard alpha band range (8-13 Hz), indicated by the grey column. *Central panel*: Fixed bandwidths may fail to adequately capture broader alpha peak distributions, thus giving rise to biased estimates of alpha centre of gravity. *Right panel*: Central panel reproduced with corresponding eyes-open resting-state spectral estimates superposed (red function). The transition frequency (i.e. the frequency at which the ascending edge of the eyes-closed alpha peak component intersects with and surpasses the eyes-open function, here indicated by the broken black line) closely approximates the lower bound alpha interval estimate $f_1$ identified via visual inspection of the plot. All power estimates log-scaled in consistency with Klimesch's [@klimesch1997] guidelines for determining the individual alpha band interval.](f1f2.png){#f1f2}
