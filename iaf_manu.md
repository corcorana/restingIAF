...
The classical method of estimating IAF relies on delineating the peak alpha frequency (PAF); a singular, prominent peak within that portion of the power spectral density (PSD) plot assumed to constitute the domain of the alpha-band frequency range ([Fig_s6_paf_poz](#s6_paf_poz)).
...
![*Fig_s6_paf_poz.* Power spectral density (PSD) plots displaying average variance of the EEG signal recorded over POz for frequency bins resolved in the range of 1 to 30 Hz. PSD estimates derived from a 2 min eyes-closed resting-state EEG recording. *Left panel*: Linear scaled PSD. Strong alpha band activity is evidenced by a sharp component spanning ~7.5 to 12.5 Hz, and peaking at ~9.75 Hz. *Right panel*: Alternative depiction of PSD displayed in the left panel, where ordinate data have been base-10 log-transformed (note, some researchers likewise tranform the frequency axis when depicting log-transformed PSDs). In this case, log-transformation has the effect of accentuating the relatively minor peak detected in the beta range of the spectrum (this activity approximates the first harmonic of the dominant alpha rhythm).](s6_paf_poz.png){#s6_paf_poz}
...
Although PAF can in many cases be rapidly ascertained upon visual inspection of the PSD function ([Fig_s6_paf_poz](#s6_paf_poz)), this approach to IAF extraction is inefficient and potentially impractical when dealing with large datasets [@goljahani2012]. 
...
![*Fig_bad_pafs.* Examples of problematic cases for peak alpha frequency identification. *Left panel*: Multiple peaks resolved within alpha band range. *Central panel*: Split- (or bimodal) alpha peak. *Right panel*: No discernable alpha peak. Note differences in the ordinate scaling of plots.](bad_pafs.png){#bad_pafs}
...
This technique exploits the typically observed anticorrelation between theta band [4–8 Hz; @noachtar1999] and alpha oscillatory dynamics in response to task demands or cognitive load [@doppelmayr1998a; @klimesch1996a; cf. @rugg1982]. 
...
<<<<<<< HEAD
![*Fig_findf2.* Application of three procedures for deducing $f_2$ (right-most broken line) to the two channel spectra presented in ([Fig_f1f2](#f1f2)). In each case, $f_1$ (left-most broken line) is defined according to the transition frequency (TF), and IAF (solid line) is defined according to the peak frequency. *Top row*: Defining $f_2$ as $IAF + 2 Hz$ (left panel) and $IAF + (IAF - TF)$ (right panel) render similar results. However, defining $f_2$ as the residual of a fixed bandwidth (6 Hz) following subtraction of the lower alpha band (blue patch) fails, collapsing $f_2$ into the IAF on account of the low TF (and a consequently broad lower alpha sub-band). *Bottom row*: All three methods perform similarly to one another. In each case, the upper alpha band (red patch) appears suboptimally narrow.](findf2.png){#findf2}
=======
![*Fig_findf2.* Application of three procedures for deducing $f_2$ to the two channel spectra presented in ([Fig_f1f2](#f1f2)). In each case, $f_1$ (left-most broken line) is defined according to the transition frequency (TF), and IAF (solid line) is defined according to the peak frequency. *Top row*: Defining $f_2$ as $IAF + 2 Hz$ (left panel) and $IAF + (IAF - TF)$ (right panel) render similar results. However, defining $f_2$ as the residual of a fixed bandwidth (6 Hz) following subtraction of the lower alpha band fails, collapsing $f_2$ into the IAF on account of the low TF (and resultant wide lower alpha bandwidth). *Bottom row*: All three methods perform similarly to one another. In each case, the upper alpha band appears suboptimally narrow.](findf2.png){#findf2}
>>>>>>> 9a34f7c65c201b08523a41ca73def68fbcebeed4
