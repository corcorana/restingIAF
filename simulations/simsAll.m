...
for snr = 1:8
    ...
    [simSnr(snr).sums, simSnr(snr).chans, f] = restingIAF(data, nbchan, cMin, fRange, Fs, w, Fw);  
...
nbchan = size(data, 1);                 % treat each simulated signal as a channel
[simN.sums, simN.chans, f] = restingIAF(data, nbchan, cMin, fRange, Fs, w, Fw);  
...
for sx = 1:6
    for ix = 1:100
        ...
        [simPar(ix, sx).sums, simPar(ix, sx).chans, f] = restingIAF(data, nbchan, cMin, fRange, Fs, w, Fw); 
...
for sx = 1:6
    for ix = 1:100
        ...
        [simBim(ix, sx).sums, simBim(ix, sx).chans, f] = restingIAF(data, nbchan, cMin, fRange, Fs, w, Fw); 
     
        % average across channel PSDs and find local maximum
        muPSD = nanmean([simBim(ix, sx).chans(:).pxx], 2);
        [simBim(ix, sx).maxi.a, simBim(ix, sx).maxi.b] = findPeak(f, muPSD, w);
    end
end
