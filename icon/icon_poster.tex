...
IAF is a fundamental property of brain processing relating to individual differences across various domains:
...
\\Further, visual identification of peak frequency from channel spectra is time-consuming and prone to bias.
...
\\PAF and CoG distributions and correlations analysed.
\\[.25cm]
\textbf{\textsc{Simulation Data}}
\\Gaussian-distributed alpha components synthesised and embedded within pink noise signal. Component dispersal (\textbf{$\alpha$}) and signal-to-noise ratio (\textbf{SNR}) parametrically varied. PAF/CoG compared to \textbf{local maximum (LM)} detection.
...
\headerbox{Future Work}{name=affils,column=3,below=results}{
  ...
  \item[--] Automate parameter settings
  \end{todolist}
}
